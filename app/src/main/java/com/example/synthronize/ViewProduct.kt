...
class ViewProduct : AppCompatActivity(), OnRefreshListener, OnNetworkRetryListener {
    ...
    private fun bindProduct() {
        ...
        FirebaseUtil().retrieveCommunityMarketCollection(communityId).document(productId).get().addOnCompleteListener {
            if (it.result.exists()){
                productModel = it.result.toObject(ProductModel::class.java)!!
